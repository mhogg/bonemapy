...
Bone properties from the bonemapy plugin can be applied to a model in an analysis using Abaqus Fortran user subroutines. As such, a Fortran compiler is required to apply the bone properties. Note that subroutines are provided for both Abaqus/Standard and Abaqus/Explicit. Details and examples are provided `here <https://github.com/mhogg/bonemapy/blob/explicit/docs/subroutines.md>`__.
